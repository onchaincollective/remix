...
function Home() {
    ...
    useEffect(() => {
        ...
        fetch("https://api.opensea.io/api/v1/assets?owner=" + account +"&order_direction=desc&offset=0&limit=20")
        .then(res => res.json())
        .then(
          (result) => {
            console.log(result);
            ...
            setWorking(false);
            let nftRendersMap = result.assets.map((nft, i) =>  
              <img 
                crossOrigin="anonymous"
                src={nft.image_url} 
                key={i}
                className="cursor-pointer w-full"
                onClick={() => loadFile(nft.image_url, true)}
                />
            );
    ...
    useEffect(() => {
      console.log('bg image changin', bgImage);
    }, [bgImage])
      ...
      getFileFromUrl(url, 'example.jpg').then((file) => {
        ...
        reader.onload = function (evt) {
          ...
          if (xmlDoc.getElementsByTagName("svg").length > 0) {
            console.log('doin some svg manipulations');
            ...
            if (y1.getAttribute('filter') == "url(#filterBG)") {
              let bg = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 500 500" height="500" width="500">'
                  + (new XMLSerializer()).serializeToString(xmlDoc.getElementById("filterBG"))
          ...
          if (bgImage) {
            ...
            fabric.Image.fromURL(output.src, function(img) {
              ...
              img3.scaleToWidth(500);
              img3.crossOrigin="Anonymous";
              // console.log(img3.getScaledHeight());
    ...
    function downloadPFP() {
      document.getElementById("svg-tag").innerHtml = canvas.toSVG();
      console.log(document.getElementById("svg-tag"));
    ...
    function refreshNfts() {
      ...
      setNfts(null);
      setNftRenders(null);
      fetch("https://api.opensea.io/api/v1/assets?owner=" + account + "&asset_contract_address=0x5a876ffc6e75066f5ca870e20fca4754c1efe91f&order_direction=desc&offset=0&limit=20")
      .then(res => res.json())
      .then(
        (result) => {
          console.log(result);
          ...
          let nftRendersMap = result.assets.map((nft, i) =>  
            <img 
              ...
              key={i} 
              className="cursor-pointer w-full"
              onClick={() => loadFile(nft.image_url, false)}
              />
          );
          setNftRenders(nftRendersMap);
          setBgImage(false);
  ...
  return (
    <main className="occ-home">
        ...
        <div className="flex flex-row space-x-8 max-w-5xl mx-auto text-center mt-10 mb-12">
          <div className="">
          ...
          {active && !working && (
            <div className="felx flex-col items-center justify-center">
              ...
              <div className="button cursor-pointer p-4 mt-8" onClick={() => refreshNfts()}>Next</div>
            </div>
          )}
        </div>
